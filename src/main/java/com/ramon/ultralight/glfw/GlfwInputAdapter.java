...
public class GlfwInputAdapter {
	public void keyCallback(long window, int key, int scancode, int action, int mods) {
		...
		if (UltralightEngine.getActiveView() == null)
			return;

		// Build the event
		UltralightKeyEvent event = new UltralightKeyEvent()
				.type(action == GLFW_PRESS || action == GLFW_REPEAT ? UltralightKeyEventType.RAW_DOWN
						: UltralightKeyEventType.UP)
				.virtualKeyCode(translatedKey).nativeKeyCode(scancode)
		...
		// Send the event
		UltralightEngine.getActiveView().fireKeyEvent(event);
		...
		if ((action == GLFW_PRESS || action == GLFW_REPEAT) && (key == GLFW_KEY_ENTER || key == GLFW_KEY_TAB)) {
			...
			UltralightKeyEvent extraEvent = new UltralightKeyEvent().type(UltralightKeyEventType.CHAR).text(text)
					.unmodifiedText(text);

			// Fire the event
			UltralightEngine.getActiveView().fireKeyEvent(extraEvent);
		}
	}
	...
	public void charCallback(long window, int codepoint) {
		...
		// Create the event
		UltralightKeyEvent event = new UltralightKeyEvent().type(UltralightKeyEventType.CHAR).text(text)
				.unmodifiedText(text);
	...
	public void cursorPosCallback(long window, double x, double y) {
		if (UltralightEngine.getActiveView() == null)
			return;
		// Create the event
		UltralightMouseEvent event = new UltralightMouseEvent().x((int) (x * 1f)).y((int) (y * 1f))
		...
		// Fire the event
		UltralightEngine.getActiveView().fireMouseEvent(event);
	}
	...
	public void mouseButtonCallback(long window, int button, int action, int mods) {
		...
		try (MemoryStack stack = MemoryStack.stackPush()) {
			...
			glfwGetCursorPos(window, (DoubleBuffer) buffer.slice().position(0),
	...
	public void scrollCallback(long window, double xDelta, double yDelta) {
		if (UltralightEngine.getActiveView() == null)
			return;
		// Create the event
		UltralightScrollEvent event = new UltralightScrollEvent().deltaX((int) xDelta * 32).deltaY((int) yDelta * 32)
				.type(UltralightScrollEventType.BY_PIXEL);

		// Fire the event
		UltralightEngine.getActiveView().fireScrollEvent(event);
	}
	...
	public void focusCallback(long window, boolean focus) {
		if (UltralightEngine.getActiveView() == null)
			return;
		if (focus) {
			UltralightEngine.getActiveView().focus();
		} else {
			UltralightEngine.getActiveView().unfocus();
		}
	}
